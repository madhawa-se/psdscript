...
ActionDescriptor.prototype.dumpDesc = function (keyName)
{
    ...
    for (i = 0; i < this.count; ++i)
    {
        ...
        if (this.getType(key) == DescValueType.LISTTYPE)
            ...
            $.writeln(keyName + "." + app.typeIDToStringID(key) + ": " + this.getFlatType(key));
    }
}
...
PSLayerInfo.prototype.gradientDesc = function ()
{
...
PSLayerInfo.prototype.gradientInfo = function ()
{
    var gradDesc = this.gradientDesc();
    ...
    if ((path.kind == PathKind.VECTORMASK) && (path.subPathItems.length == 1))
    {
        ...
        if (subPath.closed && (subPath.pathPoints.length == 4))	// Ellipse?
        {
            ...
            for (i = 0; i < 4; ++i)
            {
                ...
                if (!sameCoord(pts[i], 1 - xy))
                    return null;
                if (!near(pts[i].leftDirection[xy] - pts[i].anchor[xy],
                        (pts[next(i)].anchor[xy] - pts[i].anchor[xy]) * kEllipseDist, 100))
                    return null;
                if (!near(pts[i].anchor[xy] - pts[i].rightDirection[xy],
                        (pts[prev(i)].anchor[xy] - pts[i].anchor[xy]) * kEllipseDist, 100))
                    return null;
            }
...
cssToClip.getTextLayerCSS = function (boundsInfo)
{
    ...
    if (textDesc)
    {
        ...
        if (!fontSize && defaultDesc)
            fontSize = defaultDesc.getVal("size");
